...
import lofe.fastbytes.util.VarIntUtil;

import java.nio.charset.StandardCharsets;

public class FixedBuffer implements FastBuffer {

    protected byte[] wrapped;
    protected int dataStart, dataLength;
    ...
    public FixedBuffer(byte[] wrapped, int start, int length) {
    ...
    @Override
    public FixedBuffer writeVarInt(int val) {
        VarIntUtil.write(this, val);
        return this;
    }
    ...
    protected void writeRawBytes(byte... bytes) {
        ...
        if(dataLength < (writeCursor + bytes.length)) throwEndOfData();
        for (byte val : bytes) {
            wrapped[dataStart + writeCursor++] = val;
        }
    }
    ...
    private byte peekByte() {
        ...
        return wrapped[dataStart + readCursor++];
    }
